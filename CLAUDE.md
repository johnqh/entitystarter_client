# Entity_Starter Client

API client SDK for Entity_Starter with TanStack Query hooks.

**npm**: `@sudobility/entityentitystarter_client` (public, BUSL-1.1)
...
├── network/
│   ├── Entity_StarterClient.ts              # HTTP client class (DI-based)
│   └── Entity_StarterClient.test.ts
├── hooks/
...
└── utils/
    ...
    └── entitystarter-helpers.ts            # createAuthHeaders, buildUrl, handleApiError
```
...
- **entityentitystarter_types** — Shared type definitions; this project imports all API types (`History`, request/response types, `BaseResponse`)
- **entityentitystarter_api** — Backend server that this client SDK communicates with over HTTP
- **entityentitystarter_lib** — Business logic library that consumes this client's hooks and `Entity_StarterClient` class
- **entityentitystarter_app** — Web frontend that uses this client transitively via entityentitystarter_lib
- **entityentityentitystarter_app_rn** — React Native app that uses this client via file: links
...
- `Entity_StarterClient` class accepts `{ baseUrl, networkClient }` via constructor -- never use `fetch` directly inside this package
- Hooks (`useHistories`, `useHistoriesTotal`) wrap TanStack Query and use `Entity_StarterClient` internally
...
- Utility functions in `src/utils/entitystarter-helpers.ts` handle auth headers (`createAuthHeaders`), URL construction (`buildUrl`), and API error handling (`handleApiError`)
...
- This is a published npm package (`@sudobility/entityentitystarter_client`) -- breaking changes require version bumps and coordination with consumers
